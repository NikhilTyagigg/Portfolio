...
router.post('/login', async (req, res) => {
  ...
  try {
    ...
    const token = jwt.sign({ id: user._id, email: user.email, role: user.role }, process.env.JWT_SECRET || 'portfolio-secret', {
      expiresIn: '7d'
    });
...
router.post('/register', async (req, res) => {
  ...
  try {
    ...
    res.status(201).json({ message: 'Admin user created', user: { id: user._id, email: user.email } });
  } catch (error) {
